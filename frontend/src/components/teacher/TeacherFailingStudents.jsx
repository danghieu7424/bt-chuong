...
export default function TeacherFailingStudents({ user }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${state.domain}/api/teachers/failing-students`, {
      method: "GET",
      credentials: "include",
  ...
  return (
    <div className="teacher-failing-students">
      <h3 className="title">Danh sách sinh viên không đạt</h3>

      {grades.length === 0 ? (
        <p className="no-data">Không có sinh viên nào không đạt.</p>
      ) : (
